...
> **硬上限：每个分镜最长 12 秒（12000ms）。**
> **最优目标：每个分镜 4–5 秒。装不下就拆镜，不要拉长。**

### 时长分配指南（优选 4–5 秒）

| 内容类型 | 建议时长 | 说明 |
|----------|---------|------|
| 空镜/环境建立 | 4000-5000ms | 定场即可，不要拖沓 |
| 角色出场/亮相 | 4000-5000ms | 一个清晰亮相拍 |
| 短对话（1句） | 4000-5000ms | 约 8–15 字；更长拆镜 |
| 单招动作 | 4000-5000ms | 蓄力→发力→收势一拍 |
| 情感表情特写 | 4000-5000ms | 一个情绪落点 |

### 强制拆分规则
- 对话超过**2句**或单句过长的必须拆分
- 单镜台词宜短，装不进 5 秒就拆
- 动作序列超过一招的必须拆分（如「冲刺→拔剑→斜劈」拆 2–3 镜）
- 任何 durationMs 超过 12000 的直接视为无效输出
- durationMs 默认写 **4000 或 5000**；仅特殊情况写 6000–8000
...
- 下一镜 \`action\` 写成「起态→过程→终态」，过程必须是招式级描写（斜劈/直刺/横扫等），第一句承接上一镜收束，禁止无过渡的瞬移、换姿、换持物、换握法
  ...
  "scenes": [
    {
      "sceneNumber": 1,
      "title": "分镜标题（简洁有力）",
      "durationMs": 时长毫秒（数字，默认4000或5000，通常4000-5000，不超过12000）,
      "durationSec": 时长秒数（整数，与 durationMs 对应，默认4或5）,
